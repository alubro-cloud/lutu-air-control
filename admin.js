...
window.printQuote = function (orderId) {
    ...
    w.document.write(
        ...
        + '.bank{margin-top:8px;font-size:11px;color:#444;border-top:1px solid #ccc;padding-top:5px}'
        + '.signbox{margin-top:14px;border:1.5px solid #333;border-radius:4px;padding:10px 14px}'
        + '.signbox .t{font-weight:bold;letter-spacing:3px;margin-bottom:16px;font-size:13px}'
        + '.signbox .row{display:flex;gap:30px}'
        + '.signbox .cell{flex:1;display:flex;align-items:flex-end;gap:6px;font-size:13px}'
        + '.signbox .cell.date{max-width:210px}'
        + '.signbox .line{flex:1;border-bottom:1px solid #333;height:30px}'
        + '</style></head><body>'
        + '<h1>報　價　單</h1>'
        ...
        + '<div class="bank"><b>匯款資訊：</b>玉山銀行（808）烏日分行　帳號 1399-940-164898　戶名 鋅峰鋁業股份有限公司</div>'
        + '<div class="signbox"><div class="t">確認後請簽回</div><div class="row"><div class="cell">客戶簽章（蓋章）：<span class="line"></span></div><div class="cell date">日期：<span class="line"></span></div></div></div>'
        + '<script>window.onload=function(){setTimeout(function(){window.print()},400)}<\/script>'
        + '</body></html>'
    );
    w.document.close();
